		...
		if (player->GetFlags() & FL_FAKECLIENT)
		{
				...
				(player->GetTeamNumber() == iTeam))
			{
				...
				if (bBurn)
				{
					...
					// Live TF2 has a different burn function w/ different args so this doesnt work right now not like anyone used it anyway
...
void Bot_RunAll( void )
{
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		// Live TF2 for some reason added some new functions to check whether the player is a bot or not even though CBasePlayer::IsBot exists so i'm using that instead
		if (pPlayer && pPlayer->IsBot() && pPlayer->MyNextBotPointer() == NULL)
		{
			Bot_Think( pPlayer );
		}
	}
