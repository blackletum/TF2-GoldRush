...
const char *CTFBot::GetNextSpawnClassname( void ) const
{
	...
	*/
	...
	for( int i=0; desiredRoster[ i ].m_class != TF_CLASS_UNDEFINED; ++i )
	{
		...
		/* CanBotChooseClass probably needs class limits to work
...
//-----------------------------------------------------------------------------------------------------
void CTFBot::ChangeTeam( int iTeamNum  )
{
	// hl2_src has different CBasePlayer:: and CTFPlayer::ChangeTeam, more than 1 argument? hopefully this will not break anything
...
CCaptureFlag *CTFBot::GetFlagToFetch( void )
{
	...
	*/
	...
	for ( int i=0; i<ICaptureFlagAutoList::AutoList().Count(); ++i )
	{
		...
		/* Flag types not implemented yet
		switch( flag->GetType() )
		{
...
float CTFBot::GetThreatDanger( CBaseCombatCharacter *who ) const
{
	...
	else
	{
		...
		if ( sentry )
		{
			if ( !sentry->IsAlive() || sentry->IsPlacing() || sentry->HasSapper() || sentry->IsUpgrading() || sentry->IsBuilding() )
				return 0.0f;
...
Action< CTFBot > *CTFBot::OpportunisticallyUseWeaponAbilities( void )
{
	...
	// No weapons have abilities right now
	/*
...
{
	// TODO: fuck it, ill deal with this later i just want to get this shit working right now
